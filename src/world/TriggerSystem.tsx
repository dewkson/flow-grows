...
/**
 * Single trigger box/cylinder + resize/rotate handles when selected in editor mode. Same
 * drag/resize/rotate mechanics as ColliderBox (src/world/ColliderSystem.tsx), but this shape
 * is non-solid – it's a placement helper only, TriggerRuntime does the actual enter/exit
 * detection using the same geometry.
 */
function TriggerBox({ data }: { data: TriggerZone }) {
  ...
  // Grabbing/manipulating a trigger requires the Trigger tab to be active in the editor menu.
  const canInteract = editorMode && activeEditorPanel === 'triggers'
  ...
  const onPointerDown = useCallback(
    (handle: DragHandle) =>
      (e: ThreeEvent<PointerEvent>) => {
        ...
        setTriggerUndoSnapshot(data.id)
    ...
    [canInteract, data.id, data.position, data.size, data.radius, selectTrigger, setTriggerUndoSnapshot, setCameraDragLocked],
  )
  ...
  const onPointerMove = useCallback(
    (e: ThreeEvent<PointerEvent>) => {
      ...
      if (dragging === 'move') {
        ...
        updateTrigger(data.id, { position: [origPos[0] + dx, origPos[1] + dz] }, { recordUndo: false })
        return
      }

      if (dragging === 'rotate') {
        const angle = Math.atan2(hit.x - origPos[0], hit.z - origPos[1])
        updateTrigger(data.id, { rotationY: angle }, { recordUndo: false })
        return
      }

      if (dragging === 'radius') {
        const dist = Math.hypot(hit.x - origPos[0], hit.z - origPos[1])
        updateTrigger(data.id, { radius: Math.max(MIN_RADIUS, dist) }, { recordUndo: false })
        return
      }
      ...
      if (dragging === 'x+') {
        ...
        updateTrigger(data.id, { size: [newW, origSize[1]], position: [origPos[0] + ox, origPos[1] + oz] }, { recordUndo: false })
      } else if (dragging === 'x-') {
        const newW = Math.max(MIN_SIZE, origSize[0] - localDx)
        const [ox, oz] = localToWorldOffset(rot, -(newW - origSize[0]) / 2, 0)
        updateTrigger(data.id, { size: [newW, origSize[1]], position: [origPos[0] + ox, origPos[1] + oz] }, { recordUndo: false })
        ...
        updateTrigger(data.id, { size: [origSize[0], newD], position: [origPos[0] + ox, origPos[1] + oz] }, { recordUndo: false })
      } else if (dragging === 'z-') {
        const newD = Math.max(MIN_SIZE, origSize[1] - localDz)
        const [ox, oz] = localToWorldOffset(rot, 0, -(newD - origSize[1]) / 2)
        updateTrigger(data.id, { size: [origSize[0], newD], position: [origPos[0] + ox, origPos[1] + oz] }, { recordUndo: false })
    ...
    [dragging, data.id, data.rotationY, updateTrigger],
  )
  ...
  return (
    <group position={[data.position[0], h / 2, data.position[1]]} rotation={[0, data.rotationY, 0]}>
      ...
      <lineSegments geometry={edgesGeo}>
        <lineBasicMaterial color={isSelected ? '#c084fc' : '#818cf8'} />
      </lineSegments>

      {/* Resize/rotate handles (Trigger tab active + selected only) */}
...
/** Renders all trigger zones (editor-mode visuals + placement handles only; see TriggerRuntime for enter/exit detection) */
